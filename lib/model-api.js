...
function ModelAPI(model, options) {
	this.model = model;
	this.nameSingle = model.modelName.toLowerCase();
	this.plural = model.collection.name.toLowerCase();
...
ModelAPI.implement = function(urlMapping) {
	var map = {};
	map[this.path] = urlMapping; 
	this.mapper(map);
	return this;
}

ModelAPI.expose = function (model, options) {
	var modelAPI = new ModelAPI(model, options);
...
ModelAPI.prototype.create = function (req, res, next) {
	...
	if (req.body._id) {
		res.statusCode = 401;
		var msg = "Use post ~/"+this.plural+"/"+req.body._id+" to update a(n) "+this.nameSingle;
...
ModelAPI.prototype.findById = function (req, res, next) {
	...
	var dbQuery = this.model.findById(id, fields);
...
ModelAPI.prototype.callInstanceMethod = function(method, req, res, next) {
	this.model.findById(req.params.id, function (err, obj) {
		if (err) return next(err);
		if (!obj) {
			res.statusCode = "404";
...
ModelAPI.prototype.map = function (map) {
	...
	this.urls = [];
	var basePath = ModelAPI.path + "/"+this.plural; 
	...
	for (var i=0;i<methods.length;i++) {
		...
		if (allowedAll || allowed[methodName]) {
			...
			this.urls.push([
			                ...
			                typeof(allowed[methods[i]]) == "string"?allowed[methodName]:this.model.modelName+"."+methodName
			]);
		}
	};
