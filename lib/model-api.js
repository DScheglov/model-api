...
function ModelAPI(model, options) {
	this.model = model;
	this.querySet = options.querySet || model;
...
function buildQuery(req, restMethod) {
	...
	console.dir(query);
	var dbQuery = this.querySet.find(query, fields);
	if (skip) {
...
ModelAPI.prototype.findById = function (req, res, next) {
	...
	var dbQuery = this.querySet.findById(id, fields);
...
ModelAPI.prototype.updateById = function (req, res, next) {
	var self = this;
	var id = req.params.id;
	this.querySet.findOneAndUpdate({_id: id}, {$set: req.body}, {'new':true})
...
ModelAPI.prototype.callInstanceMethod = function(method, req, res, next) {
	this.querySet.findById(req.params.id, function (err, obj) {
		if (err) return next(err);
		if (!obj) {
			res.statusCode = "404";
