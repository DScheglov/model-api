...
module.exports = function() {
	...
	function ModelAPI(model, options) {
		...
		this.options = {
			...
			listPopulate: options.listPopulate || ModelAPI.defaults.options.listPopulate,
	...
	function buildQuery(req, restMethod) {
		...
		if (skip) {
			dbQuery = dbQuery.skip(parseInt(skip));
		}
		if (limit) {
	...
	ModelAPI.prototype.callStaticMethod = function ModelAPI__callStaticMethod (method, req, res, next) {
		var mFunc = this.model[method];
		if (!(mFunc instanceof Function)){
			return notSupported(req, res, next);
		}

		mFunc.call(this.model, req.body, function (err, obj) {
			if (err) {
				if (err.name && err.name == "ValidationError") {
					return response422(res, err);
				}
	...
	function exposeMethods(map, basePath, expose, methods, handler) {
		var self = this;
			...
			if (allowedAll || allowed[methodName]) {
				...
				map[method] = {
					"post": function(req, res, next) {
								return handler.call(self, methodName, req, res, next);
							}
				}
				this.urls.push([
		            "post "+basePath + method,
		            typeof(allowed[methods[i]]) == "string"?allowed[methodName]:this.nameSingle+"."+staticPath+methodName
				]);
			};
		};
	};
	...
	ModelAPI.prototype.map = function ModelAPI__map(map) {
		...
		exposeMethods.call(this, localMap, basePath,
						   this.options.exposeStatic, this.model.schema.statics,
		...
		exposeMethods.call(this, instanceMap, basePath,
						   this.options.expose, this.model.schema.methods,
